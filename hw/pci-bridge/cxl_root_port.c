                                               ...
                                               MemTxAttrs attrs)
{
    ...
    // memcpy(data,
    //        &cxl_mem_rw_buffer
    //             .data[cache_candidate][host_addr &
    //             CXL_MEM_ACCESS_OFFSET_MASK],
    //        size);
    return MEMTX_OK;
}
                                    ...
                                    MemTxAttrs attrs)
{
    ...
    //     trace_cxl_root_debug_message("Failed to send CXL.mem MEM RD
    //     request"); *data = 0xFF; return MEMTX_OK;
    // }
    ...
    //     trace_cxl_root_debug_message("Failed to get CXL.mem MEM DATA
    //     response"); *data = 0xFF; return MEMTX_OK;
    // }
                                     ...
                                     MemTxAttrs attrs)
{
    ...
    //     trace_cxl_root_debug_message("Failed to send CXL.mem MEM WR
    //     request"); return MEMTX_OK;
    // }
...
static void cxl_remote_device_check_device(CXLRootPort *crp, uint16_t bdf,
...
static void cxl_remote_cxl_io_mmio_write(void *opaque, hwaddr addr,
                                         uint64_t val, unsigned size)
{
    ...
    cxl_remote_rp_cxl_io_mmio_write(&crp->remote, addr, val, size);
}

static uint64_t cxl_remote_cxl_io_mmio_read(void *opaque, hwaddr addr,
                                            unsigned size)
{
    remote_memory_region_t *region = opaque;
    CXLRootPort *crp = CXL_ROOT_PORT(region->root_port);
    ...
    uint64_t val = cxl_remote_rp_cxl_io_mmio_read(&crp->remote, addr, size);
...
static void cxl_remote_device_register_memory(CXLRootPort *crp, uint16_t bdf,
                                              uint8_t bar_index)
{
    remote_memory_region_t *region =
        &crp->remote_devices[bdf].memory_regions[bar_index];
    char *name = g_strdup_printf("device-%02x-bar-%d", bdf, bar_index);
    memory_region_init_io(&region->mr, OBJECT(crp), &remote_mr_ops, region,
                          name, region->size);
...
static void cxl_remote_device_bar_update(CXLRootPort *crp, uint16_t bdf,
                                         uint32_t offset, uint32_t value,
                                         int size, bool is_write)
{
    ...
    const bool is_type0_bar =
        ranges_overlap(offset, size, PCI_BASE_ADDRESS_0, 24) && type0;
    const bool is_type1_bar =
        ranges_overlap(offset, size, PCI_BASE_ADDRESS_0, 8) && !type0;
    if (is_type0_bar) {
    ...
    if (is_write && state == REMOTE_MEMORY_UNINIT) {
        ...
        region->size = size;
        trace_cxl_root_cxl_remote_bar_size(bus, device, function, bar_index,
                                           size);
        if (size > 0) {
        ...
        trace_cxl_root_cxl_remote_bar_update_addr(bus, device, function,
                                                  bar_index, value);
    }
}

static void cxl_remote_device_update_header_type(CXLRootPort *crp, uint16_t bdf,
                                                 ...
                                                 int size)
{
    ...
    if (offset == PCI_HEADER_TYPE) {
        const bool type0 =
            (val & PCI_HEADER_TYPE_MASK) == PCI_HEADER_TYPE_NORMAL;
                                  ...
                                  uint32_t *val, int size)
{
    ...
    tlp_cpl_status_t status = cxl_remote_rp_cxl_io_cfg_read(
        &crp->remote, bdf, offset, val, (uint8_t)size, type0);
                                   ...
                                   uint32_t val, int size)
{
    ...
    tlp_cpl_status_t status = cxl_remote_rp_cxl_io_cfg_write(
        &crp->remote, bdf, offset, val, (uint8_t)size, type0);
...
static void cxl_rp_realize(DeviceState *dev, Error **errp)
{
    ...
    crp->remote.dma_address_space = &pci_dev->bus_master_as;

    trace_cxl_root_debug_message("Detected CXL Remote Root Port");
    if (!cxl_remote_rp_init(&crp->remote, crp->socket_host, crp->socket_port,
                            crp->switch_port)) {
        trace_cxl_root_error_msg("Failed to initialize CXL Remote Root Port");
        return;
    }
    ...
    for (uint32_t bdf = 0; bdf < 0x10000; ++bdf) {
        ...
        for (uint8_t bar_index = 0; bar_index < PCI_NUM_REGIONS; ++bar_index) {
            crp->remote_devices[bdf].memory_regions[bar_index].state =
                REMOTE_MEMORY_UNINIT;
            crp->remote_devices[bdf].memory_regions[bar_index].root_port =
                pci_dev;
                                ...
                                int len)
{
    ...
    // TODO: FIXME
    memory_region_set_enabled(&d->bus_master_enable_region, true);
}
